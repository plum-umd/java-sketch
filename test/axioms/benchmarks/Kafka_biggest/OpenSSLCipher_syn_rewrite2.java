...
public class OpenSSLCipher implements ICipher {
	...
	if (??) {
	    byte[] tmp = new byte[1];
	    if (localBools[0]) {
		tmp = genBytes1(localInts, localBools, localObjs);
	    }
	    @box	
	    byte[] data = tmp;
	    if (!localBools[0]) {
		data = genBytes2(localInts, localBools, localObjs);
	    }
    ...
    private void body(int[] localInts, boolean[] localBools, Object[] localObjs) {
	stmts(localInts, localBools, localObjs);

	// byte[] IV = ((Bytes) localObjs[2]).toArray();
	// SecretKeySpec key = localObjs[1];
	// localObjs[4] = getCipher(localBools[0], key, IV);	
	// Cipher cipher = (Cipher) localObjs[4];	
	// Bytes blah = localObjs[3];
	// byte[] output = blah.toArray();
	// byte[] data = new byte[1];
	// if (localBools[0]) {
	//     data = ((Bytes) localObjs[0]).toArray();
	// }
	// @box	
	// byte[] tmp = data;
	// if (!localBools[0]) {
	//     tmp = localObjs[0];
	// }
	// localInts[0] = cipher.update(tmp, 0, 1, output, 0);
	// localInts[1] = cipher.doFinale(tmp, 0, 0, output, localInts[0]);
	// Cipher c = (Cipher) localObjs[4];
	// if (!localBools[0]) {
	//     @unbox
	//     byte[] bytes = c.doFinal(tmp);
	//     localObjs[0] = new Bytes(bytes);
	// } else {
	//     localObjs[0] = c.doFinal(tmp);
	// }
    }
	...
	body(localInts, localBools, localObjs);
	...
	// byte[] bytes = c.doFinal(data);
	// localObjs[0] = new Bytes(bytes);
	byte[] blah2 = ((Bytes) localObjs[0]).toArray();
