...
with open('input_adt/test.sk') as f: test = f.read()
NUM_TESTS=int(re.findall(r't[0-9]+', test)[-1][1:])
NUM_TRIALS=int(sys.argv[1])

with open('results_adt.csv', 'w') as f: pass
with open('errors_adt.txt', 'w') as f: pass

for i in range(NUM_TESTS):
    times = []
    for j in range(NUM_TRIALS):
        cmd = ['sketch', '--fe-def', 'TID={}'.format(i), '--fe-inc', 'input_adt/', 'input_adt/main.sk']
        ...
        try:
            ...
            with open('errors_adt.txt', 'a') as f: f.write('{}\n'.format(' '.join(cmd)))
            break
